...
/**
 * Extends the {@link CardTransactionCryptoExtension} interface of the "Keypop
 * Calypso Card API" to enrich the card transaction command set with specific
 * crypto commands such as for example computation and signature verification
 * operations.
 *
 ...
 * @since 0.3.0
 */
class CardTransactionLegacySamExtension
...
public:
     ...
     *       constant signing key
     *       (which is not diversified before ciphering). Instead, the serial
     *       number of the card or ticket should be inserted at the beginning of
     *       the data to sign.
